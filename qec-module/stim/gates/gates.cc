...
void GateDataMap::add_gate(bool &failed, const Gate &gate) {
  ...
  if (!hash_loc.expected_name.empty()) {
    std::cerr << "GATE COLLISION " << gate.name << " vs "
              << items[(size_t)hash_loc.id].name << "\n";
    failed = true;
    return;
  }
