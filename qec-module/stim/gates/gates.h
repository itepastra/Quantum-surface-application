...
template <size_t W> struct Tableau;

template <size_t W> struct Flow;

template <size_t W> struct PauliString;

/// Used for gates' argument count to indicate that a gate takes a variable
/// number of arguments. This is relevant to coordinate data on detectors and
/// qubits, where there may be any number of coordinates.
constexpr uint8_t ARG_COUNT_SYGIL_ANY = uint8_t{0xFF};

/// Used for gates' argument count to indicate that a gate takes 0 parens
/// arguments or 1 parens argument. This is relevant to measurement gates, where
/// 0 parens arguments means a noiseless result whereas 1 parens argument is a
/// noisy result.
...
enum GateFlags : uint16_t {
  ...
  // Indicates whether unitary and tableau data is available for the gate, so it
  // can be tested more easily.
  GATE_IS_UNITARY = 1 << 0,
  // Determines whether or not the gate is omitted when computing a reference
  // sample.
  GATE_IS_NOISY = 1 << 1,
  ...
  // Prevents the same gate on adjacent lines from being combined into one
  // longer invocation.
  ...
  // Controls instructions like CORRELATED_ERROR taking Pauli product targets
  // ("X1 Y2 Z3").
  // Note that this enables the Pauli terms but not the combine terms like
  // X1*Y2.
  GATE_TARGETS_PAULI_STRING = 1 << 7,
  // Controls instructions like DETECTOR taking measurement record targets
  // ("rec[-1]").
  // The "ONLY" refers to the fact that this flag switches the default behavior
  // to not allowing qubit targets.
  // Further flags can then override that default.
  GATE_ONLY_TARGETS_MEASUREMENT_RECORD = 1 << 8,
  // Controls instructions like CX operating allowing measurement record targets
  // and sweep bit targets.
  ...
  GATE_IS_RESET = 1 << 13,
  // Annotations like DETECTOR aren't strictly speaking identity operations, but
  // they can be ignored by code that only
  // cares about effects that happen to qubits (as opposed to in the classical
  // control system).
...
struct Gate {
  ...
  GateType id;
  /// The id of the gate inverse to this one, or at least the closest thing to
  /// an inverse. Set to GateType::NOT_A_GATE for gates with no inverse.
  GateType best_candidate_inverse_id;
  /// The number of parens arguments the gate expects (e.g. X_ERROR takes 1,
  /// PAULI_CHANNEL_1 takes 3). Set to stim::ARG_COUNT_SYGIL_ANY to indicate any
  /// number is allowed (e.g. DETECTOR coordinate data).
  uint8_t arg_count;
  ...
  /// Prose summary of what the gate is, how it fits into Stim, and how to use
  /// it.
  ...
  /// For single qubit Cliffords, this should be the output stabilizers for X
  /// then Z. For 2 qubit Cliffords, this should be the output stabilizers for
  /// X_, Z_, _X, _Z. For 2 qubit dissipative gates, this should be flows like
  /// "X_ -> XX xor rec[-1]".
  FixedCapVector<const char *, 10> flow_data;
  /// Stim circuit file contents of a decomposition into H+S+CX+M+R operations.
  /// (nullptr if not decomposable.)
  ...
  template <size_t W> Tableau<W> tableau() const {
    if (!(flags & GateFlags::GATE_IS_UNITARY)) {
      throw std::invalid_argument(
          std::string(name) + " isn't unitary so it doesn't have a tableau.");
    }
    ...
    throw std::out_of_range(std::string(name) +
                            " doesn't have 1q or 2q tableau data.");
  }

  template <size_t W> std::vector<Flow<W>> flows() const {
  ...
  ///
  ///     The unitary matrix U of the operation can be recovered (up to global
  ///     phase) by computing U = RotZ(phi) * RotY(theta) * RotZ(lambda).
...
inline bool _case_insensitive_mismatch(std::string_view text1,
...
private:
  ...
  void add_gate_alias(bool &failed, const char *alt_name,
...
public:
  ...
  inline const Gate &operator[](GateType g) const { return items[(uint64_t)g]; }
