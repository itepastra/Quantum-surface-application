...
struct GateTarget {
  ...
  static GateTarget pauli_xz(uint32_t qubit, bool x, bool z,
                             bool inverted = false);
...
template <typename SOURCE> uint32_t read_uint24_t(int &c, SOURCE read_char) {
  if (!(c >= '0' && c <= '9')) {
    throw std::invalid_argument("Expected a digit but got '" +
                                std::string(1, c) + "'");
  }
  uint32_t result = 0;
  do {
...
inline GateTarget read_measurement_record_target(int &c, SOURCE read_char) {
  if (c != 'r' || read_char() != 'e' || read_char() != 'c' ||
      read_char() != '[' || read_char() != '-') {
...
inline GateTarget read_sweep_bit_target(int &c, SOURCE read_char) {
  if (c != 's' || read_char() != 'w' || read_char() != 'e' ||
      read_char() != 'e' || read_char() != 'p' || read_char() != '[') {
    throw std::invalid_argument("Target started with 's' but wasn't a sweep "
                                "bit argument like 'sweep[5]'.");
  }
  c = read_char();
  uint32_t lookback = read_uint24_t(c, read_char);
  if (c != ']') {
...
inline GateTarget read_single_gate_target(int &c, SOURCE read_char) {
  ...
  default:
    throw std::invalid_argument("Unrecognized target prefix '" +
                                std::string(1, c) + "'.");
  }
}
...
inline GateTarget read_pauli_target(int &c, SOURCE read_char) {
  ...
  if (c == ' ') {
    throw std::invalid_argument(
        "Pauli target '" + std::string(1, c) +
        "' followed by a space instead of a qubit index.");
  }
