...
struct Circuit {
  /// Backing data stores for variable-sized target data referenced by
  /// operations.
  ...
  // Returns one more than the largest `k` from any qubit target `k` or `!k` or
  // `{X,Y,Z}k`.
  ...
  // Returns one more than the largest `k` from any `OBSERVABLE_INCLUDE(k)`
  // instruction.
  ...
  ///     stop_asap: When set to true, the reading process stops after the next
  ///     operation is read. This is used for
  ///         interactive (repl) mode, where measurements should produce results
  ///         immediately instead of only after the circuit is entirely
  ///         specified. *This has significantly worse performance. It prevents
  ...
  /// Safely adds an operation at the end of the circuit, copying its data into
  /// the circuit's jagged data as needed.
  void safe_append(CircuitInstruction operation, bool block_fusion = false);
  /// Safely adds an operation at the end of the circuit, copying its data into
  /// the circuit's jagged data as needed.
  void safe_append_ua(std::string_view gate_name,
                      const std::vector<uint32_t> &targets,
                      double singleton_arg, std::string_view tag = "");
  /// Safely adds an operation at the end of the circuit, copying its data into
  /// the circuit's jagged data as needed.
  void safe_append_u(std::string_view gate_name,
                     const std::vector<uint32_t> &targets,
  ...
  void safe_insert_repeat_block(size_t index, uint64_t repeat_count,
                                const Circuit &block, std::string_view tag);
  void safe_insert(size_t index, const Circuit &circuit);

  /// Appends the given gate, but with targets reversed.
  void safe_append_reversed_targets(CircuitInstruction instruction,
                                    bool reverse_in_pairs);
  ...
  /// Returns a noiseless version of the given circuit. The result must live for
  /// less time than the given circuit.
  ///
  /// CAUTION: for performance, the returned circuit contains pointers into the
  /// given circuit! The result's lifetime must be shorter than the given
  /// circuit's lifetime!
  ...
  ///
  /// Args:
  ///     allow_weak_inverse: When this is set to true, the inverse of the
  ///     circuit doesn't need
  ///         to be exact. In particular, noise and measurement becomes
  ///         self-inverse. Examples:
  ...
  ///             - The weak inverse of OBSERVABLE_INCLUDE is [discard the
  ///             operation].
  ///
  /// Returns:
  ...
  /// Helper method for reverse-executing the circuit, e.g. repeating REPEAT
  /// blocks.
  template <typename CALLBACK>
  ...
  /// Looks for QUBIT_COORDS instructions in the circuit, and returns a map from
  /// qubit index to qubit coordinate.
  ///
  /// This method efficiently handles REPEAT blocks. It finishes in time
  /// proportional to the size of the circuit file, not time proportional to the
  /// amount of data produced by the circuit.
  ...
  ///         Detectors are indexed by the order they appear in the circuit
  ///         (accounting for repeat blocks).
  ///
  /// Returns:
  ...
  ///
  /// Throws:
  ///     std::invalid_argument: The detector index is greater than or equal to
  ...
  ///
  /// Args:
  ///     included_detector_indices: An ordered set of the indices of detectors
  ///     to get coordinate data for.
  ///
  /// Returns:
  ///     A map from detector index to coordinate data.
  ///     Every index from included_detector_indices will be in this map as a
  ///     key. Detectors with no coordinate data are mapped to an empty vector.
  ///
  /// Throws:
  ///     std::invalid_argument: A detector index is greater than or equal to
  ///     circuit.count_detectors().
  std::map<uint64_t, std::vector<double>> get_detector_coordinates(
      const std::set<uint64_t> &included_detector_indices) const;

  /// Returns the total coordinate shift accumulated over the entire circuit,
  /// accounting for REPEAT blocks.
  std::vector<double> final_coord_shift() const;

  /// Helper method for building up human readable descriptions of circuit
  /// locations.
...
void vec_pad_add_mul(std::vector<double> &target, SpanRef<const double> offset,
                     uint64_t mul = 1);
...
template <typename SOURCE> double read_normal_double(int &c, SOURCE read_char) {
  char buf[64];
  size_t n = 0;
...
template <typename SOURCE>
void read_tag(int &c, std::string_view name, SOURCE read_char,
              MonotonicBuffer<char> &out) {
  if (c != '[') {
    return;
  }
  c = read_char();
...
template <typename SOURCE>
void read_parens_arguments(int &c, std::string_view name, SOURCE read_char,
                           MonotonicBuffer<double> &out) {
  if (c != '(') {
    return;
  }
  c = read_char();
  ...
  read_past_within_line_whitespace(c, read_char);
  if (c != ')') {
    throw std::invalid_argument("Parens arguments for '" + std::string(name) +
                                "' didn't end with a ')'.");
  }
  c = read_char();
}
